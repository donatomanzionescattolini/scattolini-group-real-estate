

export default function ManzioneProperties() {
    return (
        <div className="row  p-0 m-0">
            <div className="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 justify-content-center p-5 text-center">
                <img
                    src={"https://pagina-mama.s3.amazonaws.com/assets2/asociados/monica.webp"}
                    alt="Elda"
                    className="w-75 m-auto img-thumbnail"
                />
            </div>
            <div className="col-xs-12 col-sm-12 col-md-6 col-lg-6 col-xl-6 p-4">
                <h2 className="text-center">Manzione Properties</h2>
                <h4 className="text-center">
                    <em> Administración de propiedades en la que puedes confiar</em>
                </h4>

                <p className="text-muted ">
                    En nuestra empresa, nos especializamos en la administración de
                    propiedades. Nos aseguramos de que cada propiedad esté bien cuidada y
                    mantenida, y nos encargamos de todos los detalles para que nuestros
                    clientes puedan disfrutar de la tranquilidad de saber que su propiedad
                    está en buenas manos.
                </p>
            </div>
        </div>
    );
}
