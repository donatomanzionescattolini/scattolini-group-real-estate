...
import {caracteristicas, ProjectParams} from "./ProjectParams.tsx";
...
export default function ProjectTemplate(props: ProjectParams) {

    const project = props.desarrollo;
    const [nombre] = useState(project.nombre);

    const [numberOfImages] = useState(project.numberOfImages);
    const [tabVisible, setTabVisible] = useState("brochure");
    const vid: string | JSX.Element = project.video
        ? project.video
        : `https://pagina-mama.s3.amazonaws.com/assets2/desarrollos/${nombre}/video.mp4`;
    const [video] = useState<string | JSX.Element>(vid);

    const [caract] = useState(project.caracteristicas as caracteristicas);
    const [titulo] = useState(project.titulo);
    const [banner] = useState(project.banner);
    const [subtitulo] = useState(project.subtitulo);
    const [introduccion] = useState(project.introduccion);
    const [CaracteristicasAmenidades] = useState(
        () => (project.caracteristicas as caracteristicas).amenidades
    );
    ...
    const [pdfUrl] = useState(`https://pagina-mama.s3.amazonaws.com/assets2/desarrollos/${nombre}/pdfs/${tabVisible}.pdf`);
    window.onresize = () => {
        ...
        {/* <embed
            ...
            <section className="white-block">
                <MDBContainer>
                    ...
                    <br></br>
                    <MDBRow
                        className="flex-row flex-wrap">{project.area.getDesarrollos().map((desarrollo: Desarrollo) => {
                        return (<MDBCol xs={12} sm={12} md={4} lg={4} xl={4}>
                            <Link to={`/desarrollos/${desarrollo.nombre}`}>
                                <div
                                    className="propiedades-img p-0 m-0"
                                    style={{
                                        background: `url('https://pagina-mama.s3.amazonaws.com/assets2/areas/${desarrollo.area.getName()}/${desarrollo.nombre}.webp')`,
                                        backgroundSize: "cover",
                                    }}
                                ></div>

                                <h4 className="text-center card-title m-2 ">{desarrollo.titulo}</h4>
                            </Link>
                        </MDBCol>)
                    })}</MDBRow>
                </MDBContainer>
